...
class EditProfile extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (this.props.match.params.id !== "create") {
      const response = await fetch(
        `http://localhost:8080/common/profile?id=${id}`,
        {
          method: "get",
          mode: "cors",
          headers: {
  ...
  async handleSubmit(event) {
    if (validateForm(this.state.errors)) {
      ...
      const res = await fetch(
        "http://localhost:8080/common/profile/update?id=" + currentUser.id,
        {
          method: "PUT",
          mode: "cors",
          headers: {
      ...
      if (res.ok) {
        return (
          alert("Thông tin cá nhân đã được thay đổi"),
          history.push('/warehouse/profile')
        )
      } else {
        return (
                      ...
                      <Col md="4">
                        <Form.Group>
                          ...
                          <select name="roleId" className="form-control" onBlur={this.handleChange} disabled>
                            <option selected={item.roleId == 4} value="4">Rescuer</option>
                            <option selected={item.roleId == 5} value="5">Warehouse</option>
                          </select>
                        </Form.Group>
                      </Col>
                    </Row>
                ...
                <Card.Body>
                  <div className="author">
                    <a href="#pablo" onClick={(e) => e.preventDefault()}>
                      ...
                      ></img>
                        : <img alt="..." className="avatar border-gray" src={require("assets/img/faces/face-8.png").default} ></img>
                      }
                      <p className="text-center">MSNV: {this.state.item.profileId}</p>
                      <p className="title" style={{ fontSize: 22 }}>{this.state.item.firstname} {this.state.item.lastname}</p>
                    </a>
                  </div>
                  <Row>
                    <Col md="6">
